...
function Landing() {
  ...
  return <main className="spade-site">
    ...
    {mode !== "browse" ? <Modal title={mode === "operator" ? "Operator entrance" : mode === "session" ? "Private reading" : mode === "join" ? "Enter a room" : "Reserve your table"} onClose={close}>{mode === "book" ? <Booking reading={selected} onCreated={(code, name) => { setSession({ code, name, role: "guest" }); setMode("session") }} /> : null}{mode === "join" ? <JoinSession onJoin={(code, name) => { setSession({ code, name, role: "guest" }); setMode("session") }} /> : null}{mode === "operator" ? <OperatorLogin initialCode={session?.code ?? ""} onAuthenticated={(code) => { setSession({ code, name: "Six", role: "operator" }); setMode("session") }} /> : null}{mode === "session" && session ? <TarotSession {...session} guestName={session.name} onLeave={close} onActivate={() => setMode("operator")} /> : null}</Modal> : null}
  </main>;
}
