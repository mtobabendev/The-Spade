...
type WireSignal = { id:number; from:string; kind:SignalKind; payload:any };
...
export function TarotSession({code, guestName, role, onLeave}:TarotSessionProps) {
  ...
  async function send(to:string,kind:SignalKind,payload:unknown){
    const r=await fetch("/api/rtc",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({op:"signal",room,from:selfId,to,kind,payload})});
  ...
  async function getMedia(){
    ...
    if(localVideo.current){localVideo.current.srcObject=stream;localVideo.current.muted=true;await localVideo.current.play().catch(()=>{})}
    return stream;
  }
  ...
  function makePeer(remoteId:string){
    ...
    pc.ontrack=e=>{
      ...
      if(remoteVideo.current){remoteVideo.current.srcObject=remote;void remoteVideo.current.play().catch(()=>{})}
    };
    pc.onicecandidate=e=>{
      if(e.candidate)void send(remoteId,"ice",e.candidate.toJSON()).catch(err=>setStatus(`ICE signaling failed: ${err instanceof Error?err.message:String(err)}`));
    };
    pc.onconnectionstatechange=()=>{
      if(pc.connectionState==="connected")setStatus("Connected");
      else if(["failed","disconnected","closed"].includes(pc.connectionState))setStatus(`Connection ${pc.connectionState}`);
  ...
  async function startGuestCall(){
    try{
      const stream=await getMedia(),pc=makePeer("host");
      stream.getTracks().forEach(t=>pc.addTrack(t,stream));
      await pc.setLocalDescription(await pc.createOffer());
      if(!pc.localDescription)throw new Error("No local offer");
      await send("host","offer",pc.localDescription.toJSON());
      setStatus("Waiting for host to press ACCEPT GUEST…");
    }catch(e){setStatus(`Call failed: ${e instanceof Error?e.message:String(e)}`)}
  }

  async function acceptGuest(){
    const call=pendingOffer.current;if(!call)return;
    setIncoming(false);setStatus("Connecting…");
    try{
      const stream=await getMedia(),pc=makePeer(call.from);
      stream.getTracks().forEach(t=>pc.addTrack(t,stream));
      await pc.setRemoteDescription(call.offer);
      await flushIce(pc);
      await pc.setLocalDescription(await pc.createAnswer());
      if(!pc.localDescription)throw new Error("No local answer");
      await send(call.from,"answer",pc.localDescription.toJSON());
      pendingOffer.current=null;
    }catch(e){setStatus(`Accept failed: ${e instanceof Error?e.message:String(e)}`)}
  }

  useEffect(()=>{
    let stopped=false,timer:number|undefined;
    void getMedia().catch(e=>setStatus(`Camera/mic failed: ${e instanceof Error?e.message:String(e)}`));
    async function poll(){
      if(stopped)return;
      try{
        const q=new URLSearchParams({room,peer:selfId,since:String(cursor.current)}),r=await fetch(`/api/rtc?${q}`,{cache:"no-store"});
        if(!r.ok)throw new Error(`Signaling failed (${r.status})`);
        const body=await r.json() as {signals:WireSignal[]};
        for(const signal of body.signals){
          cursor.current=Math.max(cursor.current,signal.id);
          if(signal.kind==="offer"&&role==="operator"){
            pendingOffer.current={from:signal.from,offer:signal.payload};setIncoming(true);setStatus("Guest is calling. Press ACCEPT GUEST.");
          }else if(signal.kind==="answer"&&role==="guest"&&peer.current){
            await peer.current.setRemoteDescription(signal.payload);await flushIce(peer.current);setStatus("Connecting…");
          }else if(signal.kind==="ice"){
            const candidate=signal.payload as RTCIceCandidateInit;
            if(peer.current?.remoteDescription)await peer.current.addIceCandidate(candidate).catch(()=>{});else pendingIce.current.push(candidate);
          }
        }
      }catch(e){setStatus(`Room connection error: ${e instanceof Error?e.message:String(e)}`)}
      if(!stopped)timer=window.setTimeout(poll,600);
    }
    void poll();
    if(role==="guest")void startGuestCall();
    return()=>{stopped=true;if(timer)clearTimeout(timer);peer.current?.close();localStream.current?.getTracks().forEach(t=>t.stop())};
  },[]);

  function toggleMute(){const t=localStream.current?.getAudioTracks()[0];if(t){t.enabled=!t.enabled;setMuted(!t.enabled)}}
  function toggleCam(){const t=localStream.current?.getVideoTracks()[0];if(t){t.enabled=!t.enabled;setCamOff(!t.enabled)}}
